...
def _genai_model(model_name: str, **kwargs):
    model_cls = getattr(genai, "GenerativeModel", None)
    if model_cls is None:
        raise AttributeError("google.generativeai.GenerativeModel is unavailable")
            ...
            if st.button("⚡ KHỞI CHẠY (AI STUDIO ENGINE)", type="primary", use_container_width=True):
                ...
                if client:
                    ...
                    try:
                        ...
                        current_user_rows = getattr(current_user_db, "data", None)

                        if isinstance(current_user_rows, list) and current_user_rows:

                            user_data = current_user_rows[0]

                            if not isinstance(user_data, dict):

                                st.error("D? li?u ng??i d?ng kh?ng h?p l?.")

                                st.stop()

                            db_role = str(user_data.get('role', ''))

                            usage_count = int(user_data.get('usage_count', 0) or 0)
            ...
            if st.button("🔄 Tải lại danh sách đề đã lưu"):
                ...
                if client:
                    ...
                    try:
                        ...
                        history_rows = getattr(history_res, "data", None)
                        if isinstance(history_rows, list) and history_rows:

                            saved_exams = [item.get('exam_data') for item in history_rows if isinstance(item, dict) and item.get('exam_data') is not None]
...
<div class="small-muted" style="margin-top:8px;">Lưu ý: Nội dung cần đúng để hệ thống SePay nhận diện.</div>
            ...
            if st.button("🚀 KÍCH HOẠT NGAY (SePay tự xác minh)", type="primary", use_container_width=True, key="dash_activate_vip"):
                ...
                if not ok:
                    ...
                    updates: dict[str, str | int] = {'role': 'pro'}
    ...
    with st.expander("💰 CHƯƠNG TRÌNH ĐỐI TÁC (AFFILIATE)", expanded=False):
        ...
        try:
            ...
            me_rows = getattr(me_res, "data", None)

            comm_balance = me_rows[0].get('commission_balance', 0) if isinstance(me_rows, list) and me_rows and isinstance(me_rows[0], dict) else 0

            ref_rows = getattr(ref_res, "data", None)

            if isinstance(ref_rows, list) and ref_rows:

                count_ref = len(ref_rows)

                count_pro = sum(1 for u in ref_rows if isinstance(u, dict) and u.get('role') == 'pro')
