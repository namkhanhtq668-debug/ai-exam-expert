...
def generate_nls_lesson_plan_legacy(api_key, lesson_content, distribution_content, textbook, subject, grade, analyze_only):
    ...
    model = _genai_model('gemini-3.5-flash-lite', system_instruction=SYSTEM_INSTRUCTION_NLS)
...
class QuestionGeneratorYCCD:
    def __init__(self, api_key):
        _genai_configure(api_key)
        # [SỬA LỖI 404] Dùng gemini-3.5-flash-lite theo yêu cầu
        self.model = _genai_model('gemini-3.5-flash-lite')
                            ...
                            if (get_user_points(client, user.get('email','')) >= 0 and get_user_points(client, user.get('email','')) < POINT_COST_EXAM):
                                ...
                                if not api_key: st.toast("⚠️ Vui lòng nhập API Key ở Tab Hồ Sơ!", icon="❌")
                                    ...
                                    with st.spinner(f"🔮 AI đang soạn đề... (Lần thứ: {usage_count + 1})"):
                                        ...
                                        try:
                                            _genai_configure(api_key)
                                            # [SỬA LỖI 404] Dùng gemini-3.5-flash-lite
                                            model = _genai_model('gemini-3.5-flash-lite', system_instruction=SYSTEM_PROMPT)
...
def _gemini_generate(prompt: str, system: str | None = None) -> str:
    ...
    try:
        _genai_configure(api_key)
        if system:
            model = _genai_model("gemini-3.5-flash-lite", system_instruction=system)
        else:
            model = _genai_model("gemini-3.5-flash-lite")
        safe_settings = [
            {"category": "HARM_CATEGORY_HARASSMENT", "threshold": "BLOCK_NONE"},
            {"category": "HARM_CATEGORY_HATE_SPEECH", "threshold": "BLOCK_NONE"},
            {"category": "HARM_CATEGORY_SEXUALLY_EXPLICIT", "threshold": "BLOCK_NONE"},
