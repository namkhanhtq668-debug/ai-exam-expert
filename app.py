...
<div>
  ...
  <div class="sb-sub">WEB AI GIÁO VIÊN</div>
    ...
    # ---- Sidebar navigation (anti-rerun / keep form inputs stable)
    # Sync radio label with current_page when navigation happens programmatically (go(...))
    if "sidebar_menu_main" not in st.session_state:
        st.session_state["sidebar_menu_main"] = current_label
    elif st.session_state["sidebar_menu_main"] != current_label:
        st.session_state["sidebar_menu_main"] = current_label
    ...
    # Only change page when user actually selects a different menu item
    if st.session_state.get("_last_menu_label") != menu_label:
        st.session_state["_last_menu_label"] = menu_label
