...
class AIPanel(ttk.Frame):
    def __init__(self, master, app, **kw):
        ...
        self.app = app
        self.config = AIConfig(api_key="", model="gemini-3.1-flash")
    ...
    def _build_settings(self):
        ...
        self.var_model = tk.StringVar(value="gemini-3.1-flash")
        cmb = ttk.Combobox(f, textvariable=self.var_model, width=30,
                           values=["gemini-3.1-flash", "gemini-3.1-flash-exp",
    ...
    def _ensure_client(self) -> GeminiClient | None:
        ...
        if not key:
            ...
            return None
        cfg = AIConfig(api_key=key, model=self.var_model.get().strip() or "gemini-3.1-flash")
        try:
